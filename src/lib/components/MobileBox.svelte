...
<Modal>
        ...
        {#if $authStateStore.authenticated}
            <div class="px-4 pt-4 mt-7 border-t">
                <div class="flex items-center gap-3">
                    ...
                    <p class="font-semibold truncate max-w-full">{$authStateStore.userDetails?.email}</p>
                </div>
    ...
    <div id="mobile-box" aria-controls="mobile-box-btn" class="fixed mobile-box overflow-y-auto overflow-x-hidden bg-white/90 dark:bg-[#20212c] text-slate-800 dark:text-slate-300 rounded-md pb-6 pe-6 shadow-lg dark:shadow-sm dark:drop-shadow-lg dark:shadow-gray-500/60 lg:left-0 lg:mx-0 lg:top-0  mb-32 lg:h-screen lg:max-w-full lg:pe-10 lg:w-full lg:absolute">
        ...
        <button 
            ...
            on:click={() => dispatch("closeMobileBox", false)} 
            class="text-3xl hidden lg:flex bg-teal-800 drop-shadow-lg shadow-md rounded-s-full p-3 pe-1 justify-center absolute items-center aspect-square w-10 h-10 my-auto right-0 top-40 bottom-0 text-gray-100 z-10 hover:bg-teal-600 box-border flex-shrink-0"
        >
            <Icon icon="mdi:arrow-collapse-left" />
        </button>
        {#if $authStateStore.authenticated}
