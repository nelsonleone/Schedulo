...
{#if showAddBoardForm}
    <Modal>
        <form  method="POST" action="?/createBoard"  on:submit|preventDefault={handleSubmit} class="add-board-form px-6 fixed w-11/12 h-[24em] z-10 fade-up overflow-y-auto mx-auto left-0 right-0 top-24 bg-white dark:bg-[#101321] rounded-md py-6 shadow-lg drop-shadow-md shadow-slate-600/80 md:w-[25em] md:h-[27em]">
            ...
            <div class="flex w-full max-w-sm flex-col my-6">
                ...
                <Input type="text" name="name" on:input={() => errors.name = null} id="board-name" placeholder="product launch" class="mt-3" />
                {#if errors.name}
                    <ErrorPara>
                        {errors.name}
                    </ErrorPara>
            ...
            <div>
                ...
                <div class="flex w-full items-center justify-between gap-3 my-3">
                    <Input type="text" name="column-{index}" on:input={() => errors.columns = null} id="column-{index}" placeholder={col} class="flex-grow" />
