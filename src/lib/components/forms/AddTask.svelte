    ...
    <Modal>
        <form  method="POST" action="?/addTask"  on:submit|preventDefault={handleSubmit} class="px-6 pt-10 fixed w-11/12 h-[24em] z-10 fade-up overflow-y-auto mx-auto left-0 right-0 top-24 bg-white dark:bg-[#101321] rounded-md py-6 shadow-lg drop-shadow-lg shadow-gray-800/60 md:w-[25em] md:h-[27em]">
            ...
            <div class="flex w-full max-w-sm flex-col my-6">
                ...
                <Input type="text" name="description" on:input={() => errors.title = null} id="description" placeholder="e.g I need to make a sketch of the structure" class="mt-3" />
            ...
            <div>
                ...
                {#each subTasksEg as val, index (index)}
                    <div class="flex w-full items-center justify-between gap-3 my-3">
                        <Input type="text" on:input={() => errors.subtasks = null} name="subtask-{index}" id="subtask-{index}" placeholder={val} class="flex-grow" />
                ...
                {/each}
                <Button type="button" on:click={() => subTasksEg = [...subTasksEg,""]} class="text-base mx-auto mt-2 block font-manrope bg-gray-100  drop-shadow-lg dark:bg-gray-200 text-light_emerald font-semibold hover:bg-gray-200/90">+ Add New Subtask</Button>
            ...
            <div class="flex flex-col gap-3 mt-5">
                <LoadingBtn type="submit" styles="text-base drop-shadow-md font-manrope bg-light_emerald text-base_color1 font-semibold hover:bg-light_emerald/70" isLoading={isCreatingBoard}>Add Task</LoadingBtn>
            </div>
        </form>
    </Modal>
{/if}